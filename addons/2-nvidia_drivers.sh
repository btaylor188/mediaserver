...
sudo apt-get update
sudo apt-get install gcc binutils make linux-source pve-headers linux-headers-$(uname -r) nvidia-docker2 nvidia-container-runtime
sudo yum install kernel-devel gcc make perl bin utils linux-source
