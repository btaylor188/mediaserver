#! /bin/bash
read -p "Did you run as sudo? " -n 1 -r
...
apt install \
	...
	ca-certificates \
	gnupg-agent \
	software-properties-common -y > install.log 2>&1
...
## add GPG key	
curl -fsSL https://download.docker.com/linux/debian/gpg | sudo apt-key add - > install.log 2>&1
## Add Repo
add-apt-repository \
   "deb [arch=amd64] https://download.docker.com/linux/debian \
   $(lsb_release -cs) \
   stable" > install.log 2>&1
## Install Docker as service and start
apt update
apt install docker-ce docker-ce-cli containerd.io -y > install.log 2>&1
...
plexinc/pms-docker > install.log 2>&1

docker network connect internal plex > install.log 2>&1
